      ...
      <div class="flex flex-col gap-lg">
        <PsCard>
          ...
          <ul class="mt-sm flex flex-col gap-xs text-body-regular">
            <li v-for="(item, i) in shipment.items" :key="i">
        ...
        <PsCard data-testid="shipment-qc-state">
          ...
          <p v-if="qcClearance.length === 0" class="mt-sm text-body-regular text-secondary">
            Keine Druckjobs — QC-Gate nicht erforderlich.
          </p>
          <ul v-else class="mt-sm flex flex-col gap-xs text-body-regular">
            <li
              v-for="c in qcClearance"
              :key="c.jobId"
              :class="c.cleared ? 'text-brand' : 'text-amber-500'"
            >
              {{ c.cleared ? '✓ freigegeben' : `⚠️ offen (${c.latestStatus ?? 'keine Prüfung'})` }}
            </li>
          </ul>
          <p
            v-if="!qcAllCleared && shipment.status === 'waiting_for_qc'"
      ...
      <div class="flex flex-col gap-lg">
        ...
        <PsCard>
          ...
          <div class="mt-md flex flex-col gap-sm">
            <a
              :href="`${pdfBase}/packing-list.pdf`"
              target="_blank"
              rel="noopener"
              data-testid="shipment-pdf-packing"
            >
              <PsButton variant="ghost" size="sm">Packliste (PDF)</PsButton>
            </a>
            <a
              :href="`${pdfBase}/delivery-note.pdf`"
              target="_blank"
              rel="noopener"
              data-testid="shipment-pdf-delivery"
            >
