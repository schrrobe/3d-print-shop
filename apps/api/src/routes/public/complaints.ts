 ...
 */
complaintsRouter.post('/', sensitiveLimiter, photoUpload.array('photos', 5), async (req, res, next) => {
  try {
...
complaintsRouter.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const input = complaintReplySchema.parse({ message: (req.body as Record<string, string>).message })
      const files = (req.files ?? []) as Express.Multer.File[]
