...
adminFilamentRouter.patch('/spools/:id', requirePermission('filament:write'), async (req, res, next) => {
  try {
...
adminFilamentRouter.delete('/spools/:id', requirePermission('filament:write'), async (req, res, next) => {
  try {
    ...
    if (!spool) throw notFound('Spool not found')
    if (spool.amsSlotAssignment) throw conflict('Spool is assigned to an AMS slot — unload it first')
  ...
  try {
    const [spools, colors] = await Promise.all([
      prisma.filamentSpool.findMany({
        where: { active: true },
        include: { color: { select: { id: true, name: true, hex: true, outOfStock: true, active: true } } },
      }),
...
adminFilamentRouter.get('/shopping-list', requirePermission('filament:read'), async (_req, res, next) => {
  try {
    const spools = await prisma.filamentSpool.findMany({
      where: { active: true },
...
adminFilamentRouter.get('/ams-units', requirePermission('filament:read'), async (_req, res, next) => {
  try {
...
adminFilamentRouter.post('/ams-units', requirePermission('filament:write'), async (req, res, next) => {
  try {
...
adminFilamentRouter.delete('/ams-units/:id', requirePermission('filament:write'), async (req, res, next) => {
  try {
    const unit = await prisma.amsUnit.findUnique({ where: { id: String(req.params.id) } })
...
adminFilamentRouter.patch('/ams-slots/:id', requirePermission('filament:write'), async (req, res, next) => {
  try {
